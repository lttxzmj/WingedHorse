...
describe("assessment scoring", () => {
  ...
  it.each(dimensions)("maps the %s reachable extremes to 0 and 100", (dimension) => {
    expect(
      scoreAssessment(questionSetV1, getExtremeAnswers(questionSetV1, dimension, "min"))
        .normalizedScores[dimension]
    ).toBe(0);
    expect(
      scoreAssessment(questionSetV1, getExtremeAnswers(questionSetV1, dimension, "max"))
        .normalizedScores[dimension]
    ).toBe(100);
  });
  ...
  it("rejects missing, unknown, and invalid answers", () => {
    ...
    expect(() => scoreAssessment(questionSetV1, { ...complete, unknown: "a" })).toThrowError(
      expect.objectContaining({ code: "UNKNOWN_QUESTION" })
    );
    expect(() => scoreAssessment(questionSetV1, { ...complete, q1: "z" })).toThrowError(
  ...
  it("returns the same result for the same answers", () => {
    const answers: AssessmentAnswers = Object.fromEntries(
      questionSetV1.questions.map((question) => [question.id, question.options[1]?.id ?? "a"])
    );
    expect(scoreAssessment(questionSetV1, answers)).toEqual(
      scoreAssessment(questionSetV1, answers)
    );
  });
});
